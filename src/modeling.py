...
class ModelingPreparer:
    ...
    def save_pipeline(self, path):
        ...
        joblib.dump({"scaler": self.scaler, "encoder": self.encoder, "target_col": self.target_col}, path)
...
class FraudModelTrainer:
    ...
    def _initialize_model(self):
        ...
        if self.model_type == "logistic":
            ...
            params = {"n_estimators": 100, "max_depth": 10, "class_weight": "balanced", "random_state": self.random_state, "n_jobs": -1}
            params.update(self.model_params)
            return RandomForestClassifier(**params)
            
        elif self.model_type == "xgboost":
            params = {"n_estimators": 100, "max_depth": 6, "learning_rate": 0.1, "scale_pos_weight": self.scale_pos_weight, 
                      "random_state": self.random_state, "use_label_encoder": False, "eval_metric": "logloss", "n_jobs": -1}
            params.update(self.model_params)
...
def run_experiment(df, target_col, strategy="smote", target_ratio=None, model_type="xgboost", threshold=0.30, model_params=None):
    ...
    X_train_res, y_train_res = preparer.apply_resampling(X_train, y_train, strategy=strategy, target_ratio=target_ratio)

    trainer = FraudModelTrainer(model_type=model_type, scale_pos_weight=calculated_weight, model_params=model_params)
...
def tune_model(X_train, y_train, X_val, y_val, model_type="xgboost", threshold=0.30):
    ...
    if model_type == "xgboost":
        for depth in [4, 6, 8]:
            for lr in [0.05, 0.1, 0.2]:
                model = XGBClassifier(max_depth=depth, learning_rate=lr, n_estimators=100,
        ...
        for depth in [5, 10, 15]:
            for est in [50, 100, 200]:
                model = RandomForestClassifier(max_depth=depth, n_estimators=est, random_state=42, n_jobs=-1)
                model.fit(X_train, y_train)
    ...
    print(f"Best {model_type.upper()} params at threshold {threshold}: {best_params} (Validation F1: {best_f1:.4f})")
    return best_params
...
def compare_models(results_list):
    ...
    print("\n" + "="*90)
    print(f"{'Model':<15} {'Strategy':<12} {'F1':<8} {'Precision':<10} {'Recall':<8} {'AUPRC':<8}")
    print("-"*90)
    for r in results_list:
        name = r.get('model_type', 'XGBoost').upper()
        print(f"{name:<15} {r['strategy']:<12} {r['f1_score']:<8.4f} {r['precision']:<10.4f} {r['recall']:<8.4f} {r['auprc']:<8.4f}")
    print("="*90)
...
def cross_validate_best_config(df, target_col, results_list, cv=5, threshold=0.30):
    ...
    print(f"Rigorous Cross-Validating: Model={model_type.upper()} | Strategy={strategy.upper()} | Ratio={target_ratio} | Threshold={threshold}")

    cols_to_drop = [target_col, "user_id", "device_id", "ip_address", "signup_time", "purchase_time", "ip_address_int"]
    X_raw = df.drop(columns=[col for col in cols_to_drop if col in df.columns])
    y_raw = df[target_col]

    X_train_raw, _, y_train_raw, _ = train_test_split(X_raw, y_raw, test_size=0.2, random_state=42, stratify=y_raw)
    ...
    for train_idx, val_idx in skf.split(X_train_df, y_train_series):
        X_tr_raw, X_val_raw = X_train_df.iloc[train_idx].reset_index(drop=True), X_train_df.iloc[val_idx].reset_index(drop=True)
        y_tr_fold, y_val_fold = y_train_series.iloc[train_idx].reset_index(drop=True), y_train_series.iloc[val_idx].reset_index(drop=True)
        ...
        num_cols = [col for col in all_nums if col not in ["is_first_4hours", "is_high_risk_country"]]
        ...
        if cat_cols:
            ...
            df_enc_tr = pd.DataFrame(encoded_tr, columns=encoded_cols, index=X_tr_raw.index)
            df_enc_val = pd.DataFrame(encoded_val, columns=encoded_cols, index=X_val_raw.index)
        ...
        if strategy == "none":
            ...
            fold_weight = counts.get(0, 1) / counts.get(1, 1) if model_type == "xgboost" else 1
            X_tr_res, y_tr_res = X_tr_fold, y_tr_fold
        else:
            fold_weight = 1
            X_tr_res, y_tr_res = fold_preparer.apply_resampling(X_tr_fold, y_tr_fold, strategy=strategy, target_ratio=target_ratio)
...
def plot_strategy_comparison(results_list, dataset_name, save_path_prefix=None):
    ...
    colors = ['#2ecc71' if s == 'smote' else '#e74c3c' if s == 'undersample' 
              else '#3498db' if s == 'oversample' else '#95a5a6' for s in strategies]
...
def plot_ratio_impact(results_list, dataset_name, strategy='smote', save_path=None):
    ...
    df_plot = pd.DataFrame([{
        ...
        'recall': r['recall'],
        'auprc': r['auprc']
    } for r in results_list if r['strategy'] == strategy]).dropna(subset=['target_ratio'])
    
    if len(df_plot) < 2:
        print(f"Not enough variation in ratio data to plot trends for {dataset_name} - {strategy}")
        return
    
    df_plot = df_plot.sort_values('target_ratio')
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    
    axes[0].plot(df_plot['target_ratio'], df_plot['f1_score'], marker='o', linewidth=2, markersize=8, label='F1 Score', color='#2ecc71')
    axes[0].plot(df_plot['target_ratio'], df_plot['precision'], marker='s', linewidth=2, markersize=8, label='Precision', color='#3498db')
    axes[0].plot(df_plot['target_ratio'], df_plot['recall'], marker='^', linewidth=2, markersize=8, label='Recall', color='#e74c3c')
    ...
    axes[1].plot(df_plot['target_ratio'], df_plot['auprc'], marker='d', linewidth=2, markersize=8, label='AUPRC', color='#9b59b6')
...
def plot_confusion_matrix_heatmap(result, dataset_name, save_path=None):
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', ax=ax, xticklabels=['Legitimate', 'Fraud'], yticklabels=['Legitimate', 'Fraud'])
    ...
    ax.text(0.5, -0.18, f'F1: {result["f1_score"]:.4f} | Precision: {result["precision"]:.4f} | Recall: {result["recall"]:.4f}', 
...
def plot_roc_curve(result, X_test, y_test, dataset_name, save_path=None):
    ...
    ax.plot(fpr[optimal_idx], tpr[optimal_idx], 'ro', markersize=9, 
    ...
    ax.text(0.35, 0.35, "Curves closer to the top-left\ncorner represent stronger\npredictive stability.", 
            transform=ax.transAxes, fontsize=9, style='italic', bbox=dict(boxstyle="round,pad=0.4", facecolor="#fff9db", edgecolor="none", alpha=0.9))
