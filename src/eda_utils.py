...
class EDAUtils:
    ...
    def plot_class_distribution(self):
        ...
        # Bar plot
        self.df[self.target].value_counts().plot(kind='bar', ax=ax1, color=['green', 'red'])
        ax1.set_title(f'{self.name} - Class Distribution')
        ax1.set_xlabel('Class')
        ax1.set_ylabel('Count')
        ax1.set_xticklabels(['Legit', 'Fraud'], rotation=0)
        
        # Pie chart
        self.df[self.target].value_counts().plot(kind='pie', ax=ax2, autopct='%1.2f%%', 
    ...
    def plot_numerical_by_target(self, numerical_cols):
        ...
        for i, col in enumerate(numerical_cols[:4]):
            for label in [0, 1]:
                ...
                axes[i].hist(subset, bins=50, alpha=0.5, label=f'Class {label}', density=True)
    ...
    def plot_categorical_by_target(self, categorical_cols):
        ...
        for i, col in enumerate(categorical_cols[:3]):
            fraud_rate = self.df.groupby(col)[self.target].mean().sort_values(ascending=False)
